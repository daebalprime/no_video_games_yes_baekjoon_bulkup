...
//		System.exit(0);
	...
	static void perm(int cnt, int[] nums,int[][] map) {
		...
		int local = 0;
		if(cnt>0) local = calculate(nums[cnt-1], newMap);
		if(answer != 0 && cnt != 0) {
			if((local) <= (answer >> (size-cnt))) return; // 가지치기
		}
		if(cnt==size) {
